...
int Count=0,OwnCount=0;
   ...
   //	 Label1->Caption = FloatToStr(RoundTo(mach,-2))+"% співпадіння";
     Application->Title="Співпадіння з еталоном";
	 ShowMessage(FloatToStr(RoundTo(mach,-2))+"% співпадіння");
...
void __fastcall TForm2::N9Click(TObject *Sender)
{
	...
	for (int j = 0; j < Try->Height; j+=10) {
		 ...
		 fout.close();
		 Application->Title="Площа контуру";
		 ShowMessage("Площа = "+IntToStr(Square)+" кв.од.");
}
//---------------------------------------------------------------------------
...
//tmp->LoadFromFile("pics\\fon.bmp");
	...
	tmp->LoadFromFile(OpenPictureDialog1->FileName);
	 ...
	 ShowMessage("Площа зображення= "+IntToStr(Square)+" кв.ум.од." +"\nПлоща еталону = " +IntToStr(EtSquare)+" кв.ум.од.");
}
//---------------------------------------------------------------------------
